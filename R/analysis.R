...
## --------------------------------------------------
load('R/data/ProvinceMapDatas.Rda')


## --------------------------------------------------
hubei<-df_China %>% filter(NAME=="湖北")


## --------------------------------------------------
plot2<-ggplot() + geom_polygon(data = df_China, aes(x = long, y = lat, group = interaction(class, group)), colour = "white", fill="seagreen1") + geom_polygon(data = hubei, aes(x = long, y = lat, group = interaction(class, group)), fill = "pink") + geom_text(aes(x=112.31, y= 31.52), label="Hubei")+
...
## --------------------------------------------------
Sys.setlocale('LC_ALL', "English")
plot3<-ggplot() + geom_point(data = shiyan, aes(x = time, y = cum_confirm, color = "Cumulative diagnosis cases in Shiyan")) + geom_point(data = tianmen, aes(x = time, y = cum_confirm, color = "Cumulative diagnosis cases in Tianmen")) + labs(y = "Cases",x="Date", color = "Legend") + scale_colour_manual( values=c(`Cumulative diagnosis cases in Shiyan` = "red", `Cumulative diagnosis cases in Tianmen` = "blue"))+
...
## --------------------------------------------------
fmyqqdata$risk[is.na(fmyqqdata$risk)] <- 0
bp2 <- fmyqqdata%>% filter(time==as.Date("2020-04-13")) %>% ggplot(aes(x=risk, y=log(cum_confirm), fill=hygc)) + geom_violin()
bp2
