...
There is 93 reconfirm hygienic cities in China in 2018 from the review of National Health Commisson of the People's Republic of China.

("怀柔区"、"平谷区"、"通州区"、"天安门地区" are analysed as "北京", so there is 90 cities in our dataset )
...
Then add the amap key and restart R
```{r}
Sys.getenv('amap.key')
```
...
Great, now we get three main datasets we need. Then next step let's combine them to one dataset
...
You see, 1 is national hygienic city, NA is not. Nice, then we replace NA as 0, who is not national hygienic city.

```{r}
mydata$hyg[is.na(mydata$hyg)] <- 0
```

Great, let's combine it with move out data.
...
I learned from my statistics class:  always visualize your data before statistics tests and modeling.

## Move out trend by date
...
Nice, then I am going to make a plot!

When make this plot, maybe we need to change locale to english, otherwise date is Chinese. 
...
I found that, make point on the line looks better, which is geom_line()+geom_point(), rather than geom_point()+geom_line()

## represent the top move out cities from Wuhan
```{r}
Sys.setlocale('LC_ALL', "Chinese")
```
...
```{r}
ggplot() + geom_polygon(data = df_China, aes(x = long, y = lat, group = interaction(class, group)), colour = "white", fill="seagreen1") + geom_polygon(data = hubei, aes(x = long, y = lat, group = interaction(class, group)), fill = "pink") + geom_text(aes(x=112.31, y= 31.52), label="Hubei")+ 
...
Don't worry, it's just one case
...
# Staistic test
```{r}
fmyqqdata%>% filter(time==as.Date("2020-04-13"))
...
ok...now let's go to linear regression
linear regression
